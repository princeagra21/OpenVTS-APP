...
extension _VehicleDetailsTab on _VehicleDetailsScreenState {
  Widget _buildVehicleDetailsTab(BuildContext context) {
    ...
    final primaryExpiry = _formatDateOnly(
      details?.raw['primaryExpiry']?.toString(),
    );
    final secondaryExpiry = _formatDateOnly(
      details?.raw['secondaryExpiry']?.toString(),
    );
    ...
    final planCurrencyRaw = plan is Map ? plan['currency']?.toString() : null;
    final planCurrencyUpper = _safe(planCurrencyRaw).toUpperCase();
    final planCurrency = planCurrencyUpper == 'INR' ? 'INR' : 'INR';
    final planDuration = _safe(
      plan is Map ? plan['durationDays']?.toString() : null,
    );
