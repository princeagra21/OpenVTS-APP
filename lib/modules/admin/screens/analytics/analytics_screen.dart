...
class _AnalyticsScreenState extends State<AnalyticsScreen> {
  ...
  double mrr = 161880.0;
  double arpu = 1349.0;
  double valueAtRisk = 9996.0;
  int renewalRate = 61;
  int churn = 39;

  final List<Map<String, dynamic>> topCustomers = [
    {"name": "Vijay Transports", "potential": 4998.0},
    {"name": "Metro Haul", "potential": 2499.0},
    {"name": "NorthStar Fleet", "potential": 2499.0},
  ];

  double collectionSuccess = 61.0;
  int atRisk = 51;
  double avgTicket = 1349.0;
  int largeCustomers = 6;
              ...
              child: Column(
                children: [
                  Row(
                    ...
                    children: [
                      _statBox("Tracked devices", tracked.toString(), "", bodyFs, smallFs, colorScheme, spacing),
                      _statBox("Expiring (<30d)", expiring.toString(), "Act now", bodyFs, smallFs, colorScheme, spacing),
                      _statBox("Overdue", overdue.toString(), "Suspension risk", bodyFs, smallFs, colorScheme, spacing),
                      _statBox("Suspended", suspended.toString(), "Disabled access", bodyFs, smallFs, colorScheme, spacing),
                    ],
                  ),
                  ...
                  Row(
                    mainAxisAlignment: MainAxisAlignment.spaceEvenly,
                    children: [
                      _statBox("MRR (est.)", f.format(mrr), "From current plans", bodyFs, smallFs, colorScheme, spacing),
                      _statBox("ARPU", f.format(arpu), "MRR / device", bodyFs, smallFs, colorScheme, spacing),
                    ],
                  ),
                ],
              ),
...
Container(
  ...
  child: Column(
    ...
    children: [
      ...
      // DEVICES TEXT
      Text(
        "$expiring devices expiring soon",
        style: GoogleFonts.inter(
          fontSize: smallFs,
          color: colorScheme.onSurface.withOpacity(0.6),
        ),
      ...
      SizedBox(
        ...
        child: ElevatedButton.icon(
          onPressed: () {},
          icon: Icon(Icons.notifications_active_outlined, color: colorScheme.primary,),
    ...
    children: [
      ...
      Row(
        ...
        children: [
          Row(
            children: [
              Icon(Icons.check_circle_outline, color: colorScheme.primary.withOpacity(0.5)),
              SizedBox(width: 8),
              Text(
      ...
      Row(
        ...
        children: [
          Row(
            children: [
              Icon(Icons.warning_amber_rounded, color: colorScheme.primary.withOpacity(0.5)),
              SizedBox(width: 8),
              Text(
                "At-risk devices",
                style: GoogleFonts.inter(
      ...
      Row(
        ...
        children: [
          Row(
            children: [
              Icon(Icons.payments_outlined, color: colorScheme.primary.withOpacity(0.5)),
              SizedBox(width: 8),
              Text(
                "Avg. ticket",
                style: GoogleFonts.inter(
      ...
      Row(
        ...
        children: [
          Row(
            children: [
              Icon(Icons.devices_other_outlined, color: colorScheme.primary.withOpacity(0.5)),
              SizedBox(width: 8),
              Text(
...
class _CashflowBoxState extends State<CashflowBox> {
  ...
  final List<double> online = [10476, 1824, 409, 12149, 4506, 4012, 3657, 2286, 12066, 1679, 11087, 12135, 14617, 8935, 1424, 9674, 6912, 520, 488, 1535, 3582, 3811, 8279, 9863, 434, 9195, 3257, 11731, 10647, 11490];
  final List<double> manual = [4464, 3436, 1805, 3679, 4827, 2278, 53, 1307, 3462, 2787, 2276, 1273, 1763, 2757, 837, 759, 3112, 792, 2940, 2817, 4945, 2166, 355, 3763, 4392, 1022, 3100, 645, 4522, 2401];
  ...
  Widget build(BuildContext context) {
    ...
    final double leftTitleFontSize = AdaptiveUtils.getTitleFontSize(screenWidth) - 3;
    final double bottomTitleFontSize = AdaptiveUtils.getTitleFontSize(screenWidth) - 2;
              ...
              LineChartData(
                ...
                lineTouchData: LineTouchData(
                  ...
                  touchCallback: (FlTouchEvent event, LineTouchResponse? response) {
                    setState(() {
                      touchedIndex = response?.lineBarSpots?.isNotEmpty == true ? response!.lineBarSpots![0].spotIndex : -1;
                    });
                  },
                  ...
                  touchTooltipData: LineTouchTooltipData(
                    getTooltipItems: (List<LineBarSpot> touchedSpots) {
                      return touchedSpots.where((spot) {
                    ...
                    sideTitles: SideTitles(
                      ...
                      getTitlesWidget: (value, meta) {
                        return Padding(
                          ...
                          child: Text(
                            NumberFormat.compactCurrency(locale: 'en_IN', symbol: '₹').format(value.toInt()),
                            style: GoogleFonts.inter(
                  ...
                  bottomTitles: AxisTitles(
                    sideTitles: SideTitles(
                      ...
                      getTitlesWidget: (value, meta) {
                        ...
                        if (index % 5 != 0 || index >= numDays) return const SizedBox();
                        final day = currentDate.subtract(Duration(days: numDays - 1 - index));
                ...
                lineBarsData: allStats.map((stat) {
                  ...
                  final isSelected = selectedStats.contains(stat);
                  final opacity = isSelected ? (stat == "Manual" ? 0.5 : 1.0) : 0.3;
                  ...
                  return LineChartBarData(
                    ...
                    barWidth: lineWidth,
                    color: color,
                    belowBarData: BarAreaData(show: true, color: baseColor.withOpacity(0.1 * opacity)),
                    dotData: FlDotData(
                      show: true,
