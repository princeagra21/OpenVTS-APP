...
class _TransactionScreenState extends State<TransactionScreen> {
  ...
  Future<void> _exportCsv(List<AdminTransactionItem> items) async {
    ...
    final filename =
        'admin_transactions_${DateTime.now().millisecondsSinceEpoch}.csv';
  ...
  Future<void> _exportPdf(List<AdminTransactionItem> items) async {
    ...
    final successCount = items.where((t) => t.normalizedStatus == 'success').length;
    final failedCount = items.where((t) => t.normalizedStatus == 'failed').length;
    final pendingCount = items.where((t) => t.normalizedStatus == 'pending').length;
    final generatedAtText = _formatDateTime(DateTime.now().toIso8601String())
        .replaceAll('\n', ' ');
    ...
    final filename =
        'admin_transactions_${DateTime.now().millisecondsSinceEpoch}.pdf';
    final dir = await _resolveDownloadDir();
    final file = File('${dir.path}${Platform.pathSeparator}$filename');
