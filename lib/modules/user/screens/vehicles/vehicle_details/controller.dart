...
import 'package:open_vts/modules/user/screens/vehicles/vehicle_details/config.dart';
import 'package:open_vts/modules/user/screens/vehicles/vehicle_details/models.dart';
...
class VehicleDetailsController extends ChangeNotifier {
  ...
  Future<VehicleDetailsActionResult?> loadDetails() async {
    _detailsToken?.cancel('Reload user vehicle details');
    ...
    final result = await repository.getVehicleDetails(
      vehicleId,
      cancelToken: token,
    );
  ...
  Future<void> loadVehicleDocuments() async {
    ...
    final result = await repository.getVehicleDocuments(
      vehicleId,
      cancelToken: token,
    );
  ...
  Future<void> loadVehicleDocumentTypes() async {
    _docTypesToken?.cancel('Reload vehicle document types');
    final token = CancelToken();
    ...
    final result = await repository.getVehicleDocumentTypes(cancelToken: token);
    if (token.isCancelled) return;
  ...
  @override
  void dispose() {
    _detailsToken?.cancel('User vehicle details disposed');
