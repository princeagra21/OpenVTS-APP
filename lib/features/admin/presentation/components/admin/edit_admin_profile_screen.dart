...
class _EditAdminProfileScreenState extends ConsumerState<EditAdminProfileScreen> {
  ...
  Future<void> _loadCurrentProfile() async {
    _ensureRepo();
    _authToken =
        await ref.read(adminAccountCommandControllerProvider).readAccessToken() ?? '';
    final res = await ref.read(adminAccountCommandControllerProvider).getMyProfile();
    if (!mounted) return;
    res.when(
      success: (profile) {
  ...
  Future<void> _pickAndUploadImage() async {
    ...
    try {
      final res = await ref.read(adminAccountCommandControllerProvider).uploadAdminFile(
        type: 'profile',
        bytes: bytes,
        filename: file.name,
  ...
  Future<void> _deleteImage() async {
    ...
    try {
      final res = await ref.read(adminAccountCommandControllerProvider).updateMyProfile(
        payload,
      );
      if (!mounted) return;
  ...
  void _snackOnce(String msg) {
    ...
    if (text.startsWith('please') ||
        ...
        text.startsWith('no ')) {
      OpenVtsFeedback.warning(context, msg);
      return;
    }
    if (text.contains('updated')) {
      OpenVtsFeedback.success(context, msg);
      return;
    }
    OpenVtsFeedback.info(context, msg);
  }
  ...
  Future<void> _submit() async {
    ...
    try {
      ...
      if (adminAccountIsAuthFailure(err)) {
