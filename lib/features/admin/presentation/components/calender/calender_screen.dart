...
class _EventCalendarScreenState extends ConsumerState<EventCalendarScreen> {
  ...
  void initState() {
    ...
    _eventSearchController.addListener(() => updateLocalUiState(this, () {}));
  ...
  Future<void> _pickMonth() async {
    ...
    updateLocalUiState(this, () => _selectedDate = month);
  ...
  Future<void> _loadDay(DateTime date) async {
    ...
    updateLocalUiState(this, () => _loadingDay = true);
    final ok = await ref.read(adminCalendarControllerProvider.notifier).loadDay(date: key);
    if (!mounted) return;
    final nextState = ref.read(adminCalendarControllerProvider);

    updateLocalUiState(this, () {
      final items = ok ? nextState.dayEvents : const <AdminCalendarEventItem>[];
      _dayCache[key] = items;
      _dayItems = items;
      _loadedDay = date;
      _loadingDay = false;
  ...
  Future<void> _loadMonth(DateTime anyDateInMonth, {bool force = false}) async {
    ...
    final ok = await ref.read(adminCalendarControllerProvider.notifier).loadMonth(
    ...
    updateLocalUiState(this, () {
      final rows = ok ? nextState.monthEvents : const <AdminCalendarEventItem>[];
    ...
    final msg = error?.statusCode == 401 || error?.statusCode == 403
                        ...
                        if (showSkeleton)
                          ...
                          CalendarDatePicker2(
                            ...
                            config: CalendarDatePicker2Config(
                              ...
                              weekdayLabelTextStyle: AppFonts.roboto(
                                fontSize: fsWeekday,
                                height: 14 / 11,
                                fontWeight: FontWeight.w500,
                                color: cs.onSurface,
                              ),
                              dayTextStyle: AppFonts.roboto(
                                fontSize: fsDate,
                                height: 20 / 14,
                                fontWeight: FontWeight.w500,
                                color: cs.onSurface,
                              ),
                              selectedDayTextStyle: AppFonts.roboto(
                                color: cs.onPrimary,
                                fontSize: fsDateSelected,
                                height: 20 / 14,
                                fontWeight: FontWeight.w600,
                              ),
                              todayTextStyle: AppFonts.roboto(
                                color: cs.primary,
                                fontSize: fsDate,
                                height: 20 / 14,
                                fontWeight: FontWeight.w600,
                              ),
                              controlsTextStyle: AppFonts.roboto(
                                fontSize: fsButton,
                                height: 20 / 14,
                                fontWeight: FontWeight.w600,
                                color: cs.onSurface,
