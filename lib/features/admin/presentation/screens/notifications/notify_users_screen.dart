...
import 'package:open_vts/features/admin/presentation/components/card/search_bar.dart';
...
class NotifyUsersScreen extends ConsumerStatefulWidget {
  const NotifyUsersScreen({super.key});

  @override
  ConsumerState<NotifyUsersScreen> createState() => _NotifyUsersScreenState();
}

class _NotifyUsersScreenState extends ConsumerState<NotifyUsersScreen> {
  final TextEditingController _searchController = TextEditingController();
  final TextEditingController _subjectController = TextEditingController();
  final TextEditingController _messageController = TextEditingController();

  /// API reference documentation (Admin section) confirmed endpoints:
  ...
  Future<void> _loadRecipients({required String query}) async {
    if (!mounted) return;
    updateLocalUiState(this, () => _loadingRecipients = true);

    final ok = await ref.read(adminNotificationsControllerProvider.notifier).loadRecipients(query: query);
    if (!mounted) return;
    final nextState = ref.read(adminNotificationsControllerProvider);

    updateLocalUiState(this, () {
      _recipients = nextState.recipients;
      _loadingRecipients = false;
      if (ok) _loadErrorShown = false;
    });

    if (!ok && !_loadErrorShown) {
      _loadErrorShown = true;
      final msg = nextState.recipientError?.message.trim().isNotEmpty == true
          ? nextState.recipientError!.message
  ...
  Future<void> _sendNotifications() async {
    ...
    final ok = await ref.read(adminNotificationsControllerProvider.notifier).send(
          channel: channel,
    ...
    final msg = nextState.actionError?.message.trim().isNotEmpty == true
        ? nextState.actionError!.message
        : "Couldn't send notification.";
    if (kDebugMode && msg.toLowerCase().contains('not available')) {
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Send API not available yet')));
      return;
    }
    ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text(msg)));
  }
          ...
          child: Column(
            ...
            children: [
                ...
                child: Row(
                  children: [
                    _channelPill(
                      ...
                      onTap: _sending
                          ? () {}
                          : () => updateLocalUiState(this, () => _channels[0] = !_channels[0]),
                      cs: cs,
                    ),
                    ...
                    _channelPill(
                      ...
                      onTap: _sending
                          ? () {}
                          : () => updateLocalUiState(this, () => _channels[1] = !_channels[1]),
                      cs: cs,
                    ),
                  ],
                ),
              ...
              _MultiSelectDropdown(
                ...
                onClear: () {
                  updateLocalUiState(this, () => _selectedUserIds.clear());
                },
              ),
              const SizedBox(height: 12),
              Card(
