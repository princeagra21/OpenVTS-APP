...
class _AdminNotificationsScreenState extends ConsumerState<AdminNotificationsScreen> {
  ProviderSubscription<AdminNotificationsState>? _notificationErrorSubscription;
  ProviderSubscription<PushNotificationState>? _pushErrorSubscription;

  @override
  void initState() {
    super.initState();
    _notificationErrorSubscription = ref.listenManual<AdminNotificationsState>(
      adminNotificationsControllerProvider,
      (previous, next) {
        final error = next.error ?? next.actionError;
        if (error == null || previous?.error == error || previous?.actionError == error || !mounted) return;
  ...
  Future<void> _loadNotifications() async {
    await ref.read(adminNotificationsControllerProvider.notifier).load();
  }
  ...
  Future<void> _markOneRead(AdminNotificationItem item) async {
    ...
    await ref.read(adminNotificationsControllerProvider.notifier).markRead(item.id);
  }

  Future<void> _markAllRead() async {
    final state = ref.read(adminNotificationsControllerProvider);
    if (state.isLoading || state.items.isEmpty) return;
    await ref.read(adminNotificationsControllerProvider.notifier).markAllRead();
  }
  ...
  Widget build(BuildContext context) {
    ...
    final notificationsState = ref.watch(adminNotificationsControllerProvider);
