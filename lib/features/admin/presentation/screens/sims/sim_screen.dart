...
import 'package:open_vts/features/admin/domain/entities/admin_sim_card_item.dart';
...
class _SimScreenState extends ConsumerState<SimScreen> {
  ...
  Future<void> _loadSimCards() async {
    ...
    try {
      final result = await _repo.getSimCards(
  ...
  Future<void> _toggleSimActive(AdminSimCardItem item, bool next) async {
    ...
    updateLocalUiState(this, () => _togglingSimIds.add(id));
    final res = await _repo.updateSimCardStatus(
      id,
      <String, dynamic>{'isActive': next},
    );
    if (!mounted) return;
    updateLocalUiState(this, () => _togglingSimIds.remove(id));
    res.when(
      success: (_) => _loadSimCards(),
      failure: (err) {
        final message = LegacyErrorPresenter.isApiFailure(err) && LegacyErrorPresenter.message(err).trim().isNotEmpty
