...
class AdminUserVehiclesTab extends ConsumerStatefulWidget {
  ...
  @override
  ConsumerState<AdminUserVehiclesTab> createState() => _AdminUserVehiclesTabState();
}
...
class _AdminUserVehiclesTabState extends ConsumerState<AdminUserVehiclesTab> {
  ...
  Future<void> _openAssignVehicleSheet() async {
    final cs = Theme.of(context).colorScheme;
    final result = await _repo.getUnlinkedVehicles(
  ...
  Future<void> _assignVehicle(AdminVehicleListItem vehicle) async {
    ...
    updateLocalUiState(this, () => _assigning = true);
    final result = await _repo.assignVehicleToUser(
      userId: widget.userId,
      vehicleId: vehicle.id,
    );
    if (!mounted) return;
    updateLocalUiState(this, () => _assigning = false);
