...
import 'package:open_vts/features/admin/presentation/components/admin/update_user_password_screen.dart';
import 'package:open_vts/features/admin/presentation/screens/account/widget/edit_company_screen.dart';
...
class AdminUserProfileTab extends ConsumerStatefulWidget {
  ...
  @override
  ConsumerState<AdminUserProfileTab> createState() => _AdminUserProfileTabState();
}

class _AdminUserProfileTabState extends ConsumerState<AdminUserProfileTab> {
  ...
  Future<void> _toggleActive(bool current) async {
    ...
    try {
      final res = await _repo.updateUserStatus(
        userId,
        !current,
      );
      if (!mounted) return;
      res.when(
        ...
        failure: (err) {
          ...
          final msg = adminAccountIsKnownFailure(err)
              ? (adminAccountErrorMessage(err).isNotEmpty
