...
class LocalizationController extends ListenableController with EffectControllerMixin {
  ...
  Future<String?> loadLocalizationData() async {
    ...
    try {
      ...
      final timezonesResult = await repository.getTimezones();
  ...
  Future<LocalizationSaveResult> saveLocalization({
    ...
    try {
      final result = await repository.updateLocalization(payload);
      ...
      return result.when(
        success: (_) async {
          ...
          await _syncGlobalSettings();
          if (showSuccess) {
            emitEffect(UiEffect.showSuccess(config.saveSuccessMessage));
            return LocalizationSaveResult.success(config.saveSuccessMessage);
          }
          return const LocalizationSaveResult.idle();
        },
        failure: (err) {
      ...
      _saveErrorShown = true;
      const message = "Couldn't save localization settings.";
      emitEffect(const UiEffect.showError(message));
      return const LocalizationSaveResult.failure(message);
    }
  }
  ...
  String _resolveLoadErrorMessage(
    ...
    if (ErrorPresenter.isAuthOrPermission(error)) {
