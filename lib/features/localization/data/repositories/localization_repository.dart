...
import 'package:open_vts/core/api/common_repository.dart' as common;
...
class LocalizationRepository {
  LocalizationRepository._({
    required common.CommonRepository commonRepository,
  ...
  factory LocalizationRepository.forRole(
    ...
    final resolvedApi = api ?? sharedLegacyTransport();
    final commonRepository = common.CommonRepository(api: resolvedApi);
  ...
  final common.CommonRepository _commonRepository;
  final _LocalizationRoleAdapter _roleAdapter;

  Future<Result<List<ReferenceOption>>> getLanguages({
    CancelToken? cancelToken,
  }) async {
    final result = await _commonRepository.getLanguages(cancelToken: cancelToken);
    return result.when(
      success: (items) => Result.ok(
        items
            .map((item) => ReferenceOption(value: item.value, label: item.label))
