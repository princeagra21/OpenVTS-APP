...
import 'package:open_vts/features/user/presentation/widgets/home/card/vehicle_status_box.dart';
...
class _DashboardScreenState extends ConsumerState<DashboardScreen> {
  ...
  late final _gateway = ref.read(userDashboardGatewayProvider);
  ...
  Future<void> _loadHome() async {
    ...
    final snapshot = await _gateway.load(
      currentFleetStatus: _fleetStatus,
      currentRecentAlerts: _recentAlerts,
      currentTopAssets: _topAssets,
    );
    if (!mounted) return;

    updateLocalUiState(this, () {
      _fleetStatus = snapshot.fleetStatus;
      _recentAlerts = snapshot.recentAlerts;
      _topAssets = snapshot.topAssets;
      _loading = false;
    ...
    if (!snapshot.hasFailure || _errorShown || !mounted) return;
    _errorShown = true;
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(content: Text(snapshot.errorMessage ?? "Couldn't load dashboard.")),
    );
  }
