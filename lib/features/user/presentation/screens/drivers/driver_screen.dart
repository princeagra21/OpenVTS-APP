...
class _DriverScreenState extends ConsumerState<DriverScreen> {
  ...
  Future<void> _loadDrivers() {
    return ref.read(userDriverListControllerProvider.notifier).load();
  }
  ...
  Widget build(BuildContext context) {
    ref.listen<UserDriverListState>(userDriverListControllerProvider, (previous, next) {
      final message = next.errorMessage;
      if (message != null && message.trim().isNotEmpty && previous?.errorMessage != message) {
        ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text(message)));
      }
    });
    final listState = ref.watch(userDriverListControllerProvider);
    final _drivers = listState.items;
