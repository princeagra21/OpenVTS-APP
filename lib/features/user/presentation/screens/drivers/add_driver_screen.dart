...
class _AddDriverScreenState extends ConsumerState<AddDriverScreen> {
  ...
  Future<void> _loadReferences() async {
    ...
    final countriesRes = await common.getCountries();
    if (!mounted) return;
    final prefixesRes = await common.getMobilePrefixes();
    if (!mounted) return;

    if (countriesRes.isSuccess && prefixesRes.isSuccess) {
      final countries = countriesRes.valueOrNull ?? const <CountryOption>[];
      final prefixes = prefixesRes.valueOrNull ?? const <MobilePrefixOption>[];
    ...
    final error = countriesRes.errorOrNull ?? prefixesRes.errorOrNull;
    final msg = error?.toString().trim().isNotEmpty == true ? error.toString() : "Couldn't load country references.";
  ...
  Future<void> _submit() async {
    ...
    final payload = <String, Object?>{
      ...
      'mobilePrefix': _normalizePrefix(_selectedPrefix?.code ?? _mobilePrefixController.text),
    ...
    final success = await ref.read(userDriverFormControllerProvider.notifier).create(payload);
    if (!mounted) return;
    ...
    final msg = ref.read(userDriverFormControllerProvider).errorMessage ?? "Couldn't add driver.";
    ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text(msg)));
  }
