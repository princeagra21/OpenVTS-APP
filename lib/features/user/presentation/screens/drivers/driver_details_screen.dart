...
import 'package:open_vts/features/user/presentation/controllers/user_driver_form_controller.dart';
...
class _DriverDetailsScreenState extends ConsumerState<DriverDetailsScreen> {
  ...
  Future<void> _loadDetails() async {
    await ref.read(userDriverFormControllerProvider.notifier).loadDetail(widget.driverId);
    if (!mounted) return;
    final controllerState = ref.read(userDriverFormControllerProvider);
    final details = controllerState.detail;
    if (details == null) {
      final msg = controllerState.errorMessage ?? "Couldn't load driver details.";
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text(msg)));
      return;
    }
    await _loadLocationLabels(details);
  }
  ...
  Future<void> _loadLocationLabels(UserDriverDetails details) async {
    ...
    final countriesRes = await ref.read(getCountriesUseCaseProvider)();
    countriesRes.when(
      success: (countries) {
    ...
    if (countryCode.isNotEmpty) {
      final statesRes = await ref.read(getStatesUseCaseProvider)(countryCode);
      statesRes.when(
        success: (states) {
  ...
  Future<void> _deleteDriver() async {
    if (ref.read(userDriverFormControllerProvider).isSubmitting) return;
    final deleted = await ref.read(userDriverFormControllerProvider.notifier).delete(widget.driverId);
    if (!mounted) return;
    if (deleted) {
      Navigator.of(context).pop(true);
      return;
    }
    final msg = ref.read(userDriverFormControllerProvider).errorMessage ?? 'Failed to delete driver.';
    ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text(msg)));
  }

  Widget _buildProfileTab(BuildContext context) {
    final controllerState = ref.watch(userDriverFormControllerProvider);
  ...
  Widget _buildDeleteTab(BuildContext context) {
    ...
    final controllerState = ref.watch(userDriverFormControllerProvider);
    final deleting = controllerState.isSubmitting;
    final limitedAccess = _isLimitedAccess(controllerState.detail);
