...
class _AddLandmarkScreenState extends ConsumerState<AddLandmarkScreen> {
  ...
  Future<void> _loadSavedLocations() async {
    ...
    final repo = ref.read(userLandmarkRepositoryProvider);
    final poisRes = await repo.getPois();
    final geofencesRes = await repo.getGeofences();
    if (!mounted) return;

    final next = <RouteLocationPreset>[];
    String? error;
    poisRes.when(
      success: (items) => next.addAll(items.map(_landmarkToDynamic).map(_poiToPreset).whereType<RouteLocationPreset>()),
      failure: (err) => error ??= _formatError(err, "Couldn't load POIs."),
    );
    geofencesRes.when(
      success: (items) => next.addAll(items.map(_landmarkToDynamic).map(_geofenceToPreset).whereType<RouteLocationPreset>()),
      failure: (err) => error ??= _formatError(err, "Couldn't load geofences."),
    );
  ...
  Map<String, dynamic> _landmarkToDynamic(UserLandmarkItem item) {
    final firstPoint = item.points.isNotEmpty ? item.points.first : null;
    return <String, dynamic>{
      'id': item.id,
      'name': item.name,
      'isActive': item.isActive,
      'color': item.colorHex,
      'type': item.shape.name,
      'radius': item.radiusMeters,
      'width': item.widthMeters,
      'updatedAt': item.updatedAt,
      'coordinates': firstPoint == null ? null : <String, dynamic>{'lat': firstPoint.latitude, 'lng': firstPoint.longitude},
      'geodata': firstPoint == null ? null : <String, dynamic>{
        'kind': item.shape.name,
        'center': <String, dynamic>{'lat': firstPoint.latitude, 'lng': firstPoint.longitude},
        'radiusM': item.radiusMeters,
        'geometry': <String, dynamic>{
          'coordinates': item.points.map((point) => <double>[point.longitude, point.latitude]).toList(growable: false),
        },
      },
    };
  }
  ...
  void _selectPreset(RouteLocationPreset preset, {bool autoFillLabel = false}) {
    updateLocalUiState(this, () => _selectedPreset = preset);
                            ...
                            Wrap(
                              ...
                              children: _iconOptions.map((icon) {
                                ...
                                return GestureDetector(
                                  onTap: () =>
                                      updateLocalUiState(this, () => _selectedIcon = icon),
                                  child: Container(
...
class _RouteLocationPickerSheet extends ConsumerStatefulWidget {
  ...
  @override
  ConsumerState<_RouteLocationPickerSheet> createState() =>
      _RouteLocationPickerSheetState();
}

class _RouteLocationPickerSheetState extends ConsumerState<_RouteLocationPickerSheet> {
            ...
            children: [
              ...
              Padding(
                ...
                child: TextField(
                  ...
                  onChanged: (value) => updateLocalUiState(this, () => _query = value),
                    ...
                    suffixIcon: _query.trim().isEmpty
                        ...
                        : IconButton(
                            onPressed: () {
                              ...
                              updateLocalUiState(this, () => _query = '');
                            },
                            icon: const Icon(Icons.close_rounded),
                          ),
                    filled: true,
