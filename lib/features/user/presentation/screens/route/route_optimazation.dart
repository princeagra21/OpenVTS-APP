...
import 'package:open_vts/features/user/presentation/screens/route/add_landmark_screen.dart';
import 'package:open_vts/features/user/presentation/screens/route/add_lat_lng_screen.dart';
import 'package:open_vts/features/user/presentation/screens/route/add_map_location_screen.dart';
import 'package:open_vts/features/user/presentation/screens/route/assign_driver_screen.dart';
...
class _WaypointSelection {
  const _WaypointSelection({
    required this.label,
    required this.icon,
    this.point,
  });
  ...
  static _WaypointSelection? fromNavigationResult(Object? value) {
    if (value is! Map) return null;
    final label = (value['label'] ?? '').toString().trim();
    if (label.isEmpty) return null;
    final lat = value['lat'];
    final lng = value['lng'];
    final point = lat is num && lng is num ? LatLng(lat.toDouble(), lng.toDouble()) : null;
    return _WaypointSelection(
      label: label,
      icon: value['icon'] is IconData ? value['icon'] as IconData : Icons.location_on,
      point: point,
    );
  }
}
...
class _RouteOptimizationScreenState extends ConsumerState<RouteOptimizationScreen> {
  ...
  void initState() {
    ...
    Future.microtask(() => ref.read(userRouteControllerProvider.notifier).loadRoutes());
  }
  ...
  void _toggleMoreMenu() {
    updateLocalUiState(this, () => _isMoreMenuOpen = !_isMoreMenuOpen);
  }

  void _closeMoreMenu() {
    if (!_isMoreMenuOpen) return;
    updateLocalUiState(this, () => _isMoreMenuOpen = false);
  }
  ...
  bool _isRouteBusy() => ref.read(userRouteControllerProvider).isBusy;

  void _syncRoutePointsToController() {
    ref.read(userRouteControllerProvider.notifier).setRoutePoints(
          _waypoints.map((w) => w.point).toList(growable: false),
        );
  }

  Future<void> _clearWaypoints() async {
    final selectedRouteId = ref.read(userRouteControllerProvider).selectedRoute?.id ?? '';
    final cleared = await ref.read(userRouteControllerProvider.notifier).deleteRoute(selectedRouteId);
    if (!mounted || !cleared) return;
    updateLocalUiState(this, _resetRouteState);
  }
  ...
  Future<void> _optimizeRoute() async {
    final points = _waypoints.map((w) => w.point).toList(growable: false);
    final optimized = await ref.read(userRouteControllerProvider.notifier).optimizeRoute(
          points: points,
          name: _routeName,
        );
    if (!mounted || !optimized) return;

    final result = ref.read(userRouteControllerProvider).optimizationResult;
    if (result == null) return;
    updateLocalUiState(this, () {
      _route = List<LatLng>.from(result.points);
      _isOptimized = true;
  ...
  Future<void> _handleAdd(String type, BuildContext ctx) async {
    ...
    final result = _WaypointSelection.fromNavigationResult(
    ...
    if (type == 'map_location') {
      _pendingLabel = result.label;
      _pendingIcon = result.icon;

      updateLocalUiState(this, () => _isPickingFromMap = true);
      ...
      _addWaypoint(point, label: label, icon: icon);
      _mapController.move(point, _routeFocusZoom);
      updateLocalUiState(this, () => _currentZoom = _routeFocusZoom);
  ...
  Future<void> _assignDriver() async {
    final current = ref.read(userRouteControllerProvider).assignedDriver;
    final selected = await Navigator.push<String?>(
      context,
      MaterialPageRoute(
        builder: (_) => AssignDriverScreen(current: current),
      ),
    );

    if (!mounted || selected == null || selected == current) return;
    ref.read(userRouteControllerProvider.notifier).setAssignedDriver(selected);
    ScaffoldMessenger.of(
      context,
  ...
  String _routeSummaryBody() {
    final assignedDriver = ref.read(userRouteControllerProvider).assignedDriver;
    final lines = <String>[
      'Route: $_routeName',
      'Distance: ${_totalDistanceKm.toStringAsFixed(2)} km',
      'Waypoints: ${_waypoints.length}',
      if ((assignedDriver ?? '').trim().isNotEmpty)
        'Assigned Driver: ${assignedDriver!.trim()}',
      '',
      'Stops:',
    ];
  ...
  @override
  Widget build(BuildContext context) {
    final routeState = ref.watch(userRouteControllerProvider);
    ref.listen<UserRouteState>(userRouteControllerProvider, (previous, next) {
      final effect = next.effect;
      if (effect != null && !identical(previous?.effect, effect)) {
        ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text(effect.message)));
        ref.read(userRouteControllerProvider.notifier).clearEffect();
      }

      final selected = next.selectedRoute;
      if (selected != null && previous?.selectedRoute?.id != selected.id) {
        updateLocalUiState(this, () => _hydrateFromRoute(selected));
      }
    });

    final isLoading = routeState.isLoading;
    final isSaving = routeState.isSaving;
    final isDeleting = routeState.isDeleting;
    final isOptimizing = routeState.isOptimizing;
    final isBusy = routeState.isBusy;
    final assignedDriver = routeState.assignedDriver;
                      ...
                      children: [
                        _buildToolChip(
                          ...
                          onPressed:
                              ...
                              ? null
                              : () async {
                                  updateLocalUiState(this, () => _activeAddMode = 'landmark');
                                  await _handleAdd('landmark', context);
                                  if (mounted) {
                                    updateLocalUiState(this, () => _activeAddMode = null);
                                  }
                                },
                        ),
                        ...
                        _buildToolChip(
                          ...
                          onPressed:
                              ...
                              ? null
                              : () async {
                                  updateLocalUiState(this, () => _activeAddMode = 'lat_lng');
                  ...
                  child: Card(
                    child: Padding(
                      ...
                      child: Column(
                        crossAxisAlignment: CrossAxisAlignment.start,
                        children: [
                          if (isLoading || isSaving || isDeleting || isOptimizing) ...[
