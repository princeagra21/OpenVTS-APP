...
extension _GeofenceSheets on _GeofenceScreenState {
                            ...
                            itemBuilder: (_, index) {
                              ...
                              return InkWell(
                                onTap: () {
                                  ...
                                  updateLocalUiState(this, () => _selectedLandmark = g);
                                },
  ...
  Future<void> _showLandmarkInfoForHits(List<Geofence> hits) async {
    ...
    updateLocalUiState(this, () => _selectedLandmark = hits.first);
    await showModalBottomSheet<void>(
      context: context,
      isScrollControlled: true,
      useSafeArea: true,
    ...
    if (mounted) updateLocalUiState(this, () => _selectedLandmark = null);
  }


  String _geofenceTypeLabel(GeofenceType type) {
    switch (type) {
  ...
  LatLng _centerOfPoints(List<LatLng> points) {
    if (points.isEmpty) return _currentCenter;
  ...
  Future<void> _showLandmarkInfo(Geofence landmark) => _showLandmarkInfoForHits(<Geofence>[landmark]);

  void _focusLandmarkOnMap(Geofence landmark) {
    if (landmark.points.isEmpty) return;
    final center = _centerOfPoints(landmark.points);
    _mapController.move(center, _GeofenceScreenState._focusMapZoom);
    updateLocalUiState(this, () {
      _currentCenter = center;
      _currentZoom = _GeofenceScreenState._focusMapZoom;
      _selectedLandmark = landmark;
      _hasUserInteractedWithMap = true;
    });
  }

  IconData _landmarkIcon(Geofence landmark) {
  ...
  String _landmarkSummary(Geofence landmark) {
    final measure = _landmarkMeasureValue(landmark);
    return '${_geofenceTypeLabel(landmark.type)} • $measure';
  }

  String _landmarkMeasureLabel(Geofence landmark) {
    switch (landmark.type) {
      case GeofenceType.circle:
        return 'Radius';
      case GeofenceType.line:
  ...
  String _landmarkMeasureValue(Geofence landmark) {
    switch (landmark.type) {
      case GeofenceType.circle:
        return '${(landmark.radius ?? 0).toStringAsFixed(0)} m';
      case GeofenceType.line:
      case GeofenceType.route:
        return '${_polylineDistanceMeters(landmark.points).toStringAsFixed(0)} m';
      case GeofenceType.rectangle:
      case GeofenceType.polygon:
        return '${landmark.points.length} points';
      case GeofenceType.poi:
        if (landmark.points.isEmpty) return '-';
        final p = landmark.points.first;
