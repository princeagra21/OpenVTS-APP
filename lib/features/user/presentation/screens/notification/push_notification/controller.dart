...
class PushNotificationController extends ListenableController {
  ...
  Future<PushNotificationActionResult?> saveOverspeedChanges() async {
    ...
    res.when(
      ...
      failure: (error) {
        final message = error.toString().trim().isNotEmpty ? error.toString() : "Couldn't save overspeed preferences.";
  ...
  Future<PushNotificationActionResult?> toggleGeofenceRule({
    ...
    if (existing is List) {
      for (final item in existing) {
        if (item is Map) {
          matrix.add(<String, Object?>{for (final entry in item.entries) entry.key.toString(): entry.value});
        }
      }
    }
  ...
  PushNotificationActionResult? _errorResult(
    ...
    return PushNotificationActionResult.error((message == null || message.isEmpty) ? fallback : message);
  }
