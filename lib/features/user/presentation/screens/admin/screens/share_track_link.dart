...
import 'package:open_vts/features/user/domain/entities/user_share_track_link_item.dart';
...
class ShareTrackScreen extends ConsumerStatefulWidget {
  const ShareTrackScreen({super.key});

  @override
  ConsumerState<ShareTrackScreen> createState() => _ShareTrackScreenState();
}

class _ShareTrackScreenState extends ConsumerState<ShareTrackScreen> {
  ...
  int _pageSize = 10;
  final Map<String, int> _toggleRequestIds = <String, int>{};
  final Map<String, int> _deleteRequestIds = <String, int>{};
  late final _repo = ref.read(userShareTrackLinksAccessProvider);
  ...
  Future<void> _loadLinks() async {
    ...
    final result = await _repo.getLinks();
    ...
    result.when(
      ...
      failure: (error) {
        ...
        final msg = ErrorPresenter.message(
          error,
          fallback: "Couldn't load share links.",
        );
  ...
  Future<void> _toggleLink(UserShareTrackLinkItem item) async {
    ...
    final requestId = (_toggleRequestIds[item.id] ?? 0) + 1;
    _toggleRequestIds[item.id] = requestId;
    ...
    final result = await _repo.setLinkActive(item.id, next);
    if (!mounted || _toggleRequestIds[item.id] != requestId) return;
    ...
    result.when(
      ...
      failure: (error) {
        ...
        if (ErrorPresenter.isCancellation(error)) return;
        final msg = ErrorPresenter.message(
          error,
          fallback: "Couldn't update link status.",
        );
        ScaffoldMessenger.of(
          context,
        ).showSnackBar(SnackBar(content: Text(msg)));
  ...
  Future<void> _deleteLink(UserShareTrackLinkItem item) async {
    ...
    final requestId = (_deleteRequestIds[item.id] ?? 0) + 1;
    _deleteRequestIds[item.id] = requestId;

    if (!mounted) return;
    updateLocalUiState(this, () => _deletingIds.add(item.id));

    final result = await _repo.deleteLink(item.id);
    if (!mounted || _deleteRequestIds[item.id] != requestId) return;
    ...
    result.when(
      ...
      failure: (error) {
        updateLocalUiState(this, () => _deletingIds.remove(item.id));
        if (ErrorPresenter.isCancellation(error)) return;
        final msg = ErrorPresenter.message(
          error,
          fallback: "Couldn't delete share link.",
        );
        ScaffoldMessenger.of(
          context,
        ).showSnackBar(SnackBar(content: Text(msg)));
