...
import 'package:open_vts/shared/widgets/status_pill.dart';
...
class SubUserScreen extends ConsumerStatefulWidget {
  final bool embedded;

  const SubUserScreen({super.key, this.embedded = false});

  @override
  ConsumerState<SubUserScreen> createState() => _SubUserScreenState();
}

class _SubUserScreenState extends ConsumerState<SubUserScreen> {
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) => _loadSubUsers());
  }
  ...
  Future<void> _loadSubUsers() {
    return ref.read(userSubUserListControllerProvider.notifier).load(limit: _pageSize);
  }
  ...
  Widget build(BuildContext context) {
    ref.listen<UserSubUserListState>(userSubUserListControllerProvider, (previous, next) {
      final message = next.errorMessage;
      if (message != null && message.trim().isNotEmpty && previous?.errorMessage != message) {
        ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text(message)));
      }
    });
    final listState = ref.watch(userSubUserListControllerProvider);
    final _subUsers = listState.items;
    final _loading = listState.isLoading;
