...
import 'package:open_vts/features/user/presentation/controllers/user_sub_user_form_controller.dart';
import 'package:open_vts/core/theme/app_fonts.dart';
import 'package:open_vts/core/state/update_local_ui_state.dart';

class AddSubUserScreen extends ConsumerStatefulWidget {
  ...
  @override
  ConsumerState<AddSubUserScreen> createState() => _AddSubUserScreenState();
}

class _AddSubUserScreenState extends ConsumerState<AddSubUserScreen> {
  ...
  ReferenceDataRepository _commonOrCreate() {
  ...
  Future<void> _loadDetails() async {
    ...
    if (!mounted) return;
    await ref.read(userSubUserFormControllerProvider.notifier).loadDetail(widget.subUserId!);
    if (!mounted) return;
    final formState = ref.read(userSubUserFormControllerProvider);
    final details = formState.detail;
    if (details != null) {
      _applyInitialValues(details);
      return;
    }
    final message = formState.errorMessage;
  ...
  Future<void> _loadPrefixes() async {
        if (!mounted) return;
    updateLocalUiState(this, () => _loadingRefs = true);

    try {
      final res = await _commonOrCreate().getMobilePrefixes();
      if (!mounted) return;
      res.when(
        success: (items) {
          String? selected;
  ...
  Future<void> _loadCountries() async {
    ...
    final res = await _commonOrCreate().getCountries();
    if (!mounted) return;
  ...
  Future<void> _loadStates(String countryCode) async {
    ...
    final res = await _commonOrCreate().getStates(countryCode, );
    if (!mounted) return;
    ...
    res.when(
      ...
      failure: (error) {
        ...
        final msg = "Couldn't load states.";
        ScaffoldMessenger.of(
          context,
        ).showSnackBar(SnackBar(content: Text(msg)));
      },
  ...
  Future<void> _loadCities(String countryCode, String stateCode) async {
    ...
    res.when(
      ...
      failure: (error) {
        ...
        final msg = "Couldn't load cities.";
        ScaffoldMessenger.of(
          context,
        ).showSnackBar(SnackBar(content: Text(msg)));
      },
  ...
  Future<void> _save() async {
    if (ref.read(userSubUserFormControllerProvider).isSubmitting) return;
    ...
    final payload = <String, Object?>{
      ...
      'mobileNumber': _mobileController.text.replaceAll(RegExp(r'\D'), '').trim(),
      'isActive': _isActive,
    };
    ...
    final controller = ref.read(userSubUserFormControllerProvider.notifier);
    final success = _isEditMode
        ? await controller.update(widget.subUserId!, payload)
    ...
    final message = ref.read(userSubUserFormControllerProvider).errorMessage ?? "Couldn't add sub-user.";
    ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text(message)));
  }
                ...
                child: Form(
                  ...
                  child: SingleChildScrollView(
                    ...
                    child: Column(
                      crossAxisAlignment: CrossAxisAlignment.start,
                      children: [
                        if (ref.watch(userSubUserFormControllerProvider).isLoading && isEdit)
                          const AppShimmer(
                        ...
                        SwitchListTile.adaptive(
                          value: _isActive,
                          onChanged: (value) =>
                              updateLocalUiState(this, () => _isActive = value),
              ...
              Expanded(
                child: SizedBox(
                  ...
                  child: OutlinedButton(
                    onPressed: ref.watch(userSubUserFormControllerProvider).isSubmitting ? null : () => Navigator.pop(context),
              ...
              Expanded(
                child: SizedBox(
                  ...
                  child: ElevatedButton(
                    onPressed: ref.watch(userSubUserFormControllerProvider).isSubmitting ? null : _save,
                    ...
                    child: ref.watch(userSubUserFormControllerProvider).isSubmitting
