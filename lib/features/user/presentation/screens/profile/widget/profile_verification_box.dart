...
class _ProfileVerificationBoxState extends ConsumerState<ProfileVerificationBox> {
  ...
  AppCancellationHandle? _sendEmailToken;
  AppCancellationHandle? _sendPhoneToken;
  ...
  Future<void> _sendOtp(_VerificationChannel channel) async {
    ...
    try {
      _repo ??= ref.read(userProfileRepositoryAdapterProvider);
      ...
      result.when(
        ...
        failure: (error) {
          ...
          if (LegacyErrorPresenter.isApiFailure(error)) {
            if (LegacyErrorPresenter.statusCode(error) == 401 || LegacyErrorPresenter.statusCode(error) == 403) {
              msg = 'Not authorized to request verification.';
            } else if (LegacyErrorPresenter.message(error).trim().isNotEmpty) {
...
class _OtpVerifySheet extends ConsumerStatefulWidget {
  ...
  final String title;
  final Future<Result<void>> Function(String code, AppCancellationHandle token) onVerify;
