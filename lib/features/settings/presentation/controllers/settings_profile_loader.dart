...
class SettingsProfileDataLoader {
  ...
  SettingsProfileDataLoader copyWith({
    void Function(dynamic profile)? onProfileLoaded,
  }) {
    return SettingsProfileDataLoader(
      adminRepo: adminRepo,
      userRepo: userRepo,
      superadminRepo: superadminRepo,
      onProfileLoaded: onProfileLoaded ?? this.onProfileLoaded,
    );
  }
  ...
  String _extractProfileImageUrl(Object? rawValue) {
    final raw = _toDynamicMap(rawValue);
  ...
  String _extractWhatsapp(Object? rawValue) {
    final raw = _toDynamicMap(rawValue);
    final sources = <Map<String, dynamic>>[raw];
    final level1 = raw['data'];
    if (level1 is Map) {
