...
import 'package:open_vts/features/settings/widgets/settings_group.dart';
...
import 'package:open_vts/modules/admin/components/admin/edit_admin_profile_screen.dart'
    as admin_profile;
...
class RoleAwareSettingsScreen extends StatefulWidget {
  const RoleAwareSettingsScreen({super.key, required this.role});

  final SettingsRole role;

  @override
  State<RoleAwareSettingsScreen> createState() =>
      _RoleAwareSettingsScreenState();
}

class _RoleAwareSettingsScreenState extends State<RoleAwareSettingsScreen> {
  ...
  SettingsRoleConfig _configForRole(SettingsRole role) {
    switch (role) {
  ...
  Future<void> _onEditProfile() async {
    switch (widget.role) {
        ...
        final changed = await Navigator.push<bool>(
          ...
          MaterialPageRoute(
            builder: (_) => admin_profile.EditAdminProfileScreen(
              initialProfile: _adminOrUserProfile,
            ),
          ),
        );
        if (changed == true) {
      ...
      case SettingsRole.superadmin:
        final profileId = _controller.profile.profileId;
        if (profileId.isEmpty) return;
        await Navigator.push<void>(
          context,
          MaterialPageRoute(
            builder: (_) =>
                superadmin_profile.EditAdminProfileScreen(adminId: profileId),
          ),
        );
        return;
    }
    ...
    switch (widget.role) {
      ...
      case SettingsRole.superadmin:
        ...
        await Navigator.push<void>(
          ...
          MaterialPageRoute(
            builder: (_) =>
                superadmin_password.UpdatePasswordScreen(adminId: profileId),
          ),
        );
        return;
    }
  ...
  Future<void> _sendAndVerifyAdminOtp(_VerifyChannel channel) async {
    _ensureAdminRepo();
  ...
  Future<void> _sendAndVerifyUserOtp(_VerifyChannel channel) async {
    _ensureUserRepo();
    await _sendAndVerifyOtp(
      title: channel == _VerifyChannel.email
          ? 'Verify Email'
    ...
    final bool? verified = await sendRes.when(
      success: (_) async {
        return showModalBottomSheet<bool>(
          ...
          builder: (_) => _OtpVerifySheet(title: title, onVerify: verifyOtp),
        );
      },
      failure: (error) async {
    ...
    res.when(
      success: (data) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text(
              _responseMessage(data, fallback: 'Email OTP request sent.'),
            ),
          ),
        );
      },
  ...
  Widget _buildProfileSection(SettingsProfileData profile, bool loading) {
    final onEmailVerify = switch (widget.role) {
      SettingsRole.admin => () => _sendAndVerifyAdminOtp(_VerifyChannel.email),
      SettingsRole.user => () => _sendAndVerifyUserOtp(_VerifyChannel.email),
      SettingsRole.superadmin => _requestEmailOtp,
    };

    final onPhoneVerify = switch (widget.role) {
      SettingsRole.admin => () => _sendAndVerifyAdminOtp(
        _VerifyChannel.whatsapp,
      ),
      SettingsRole.user => () => _sendAndVerifyUserOtp(_VerifyChannel.whatsapp),
    ...
    return Column(
      children: [
        SettingsProfileHeader(
          ...
          emailActionLoading: _emailOtpLoading,
          phoneActionLoading: _whatsappOtpLoading,
        ),
        if (widget.role == SettingsRole.superadmin) ...[
                    ...
                    child: Column(
                      ...
                      children: [
                        SettingsGroup(
                          ...
                          sections: _config.availableSections,
    ...
    return AlertDialog(
      ...
      actions: [
        TextButton(
          ...
          child: Text('Cancel', style: AppFonts.roboto(color: cs.onSurface)),
        ),
        ElevatedButton(
          ...
          child: Text('Enable', style: AppFonts.roboto(color: cs.onPrimary)),
        ),
      ],
    );
  }
    ...
    return AlertDialog(
      ...
      actions: [
        ...
        ElevatedButton(
          ...
          child: Text('Send', style: AppFonts.roboto(color: cs.onPrimary)),
        ),
      ],
    );
  }
