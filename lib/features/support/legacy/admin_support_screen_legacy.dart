...
class _SupportScreenState extends State<SupportScreen> {
  ...
  Future<void> _loadTickets({bool forMyTickets = false}) async {
    ...
    final result = forMyTickets
        ? await _repoOrCreate().getMyTickets(limit: 100, cancelToken: token)
  ...
  Widget build(BuildContext context) {
    ...
    final tickets =
        (showingMyTickets ? _myTickets : _tickets) ??
...
class _TicketDetailsScreenState extends State<TicketDetailsScreen> {
  ...
  Future<void> _openFullscreenChat() async {
    ...
    final String fromName =
        ((fromUserMap['name'] ?? '').toString().trim().isNotEmpty)
  ...
  Widget build(BuildContext context) {
    ...
    String fromEmail = '—';
    final rawEmail =
        (fromUserMap.isNotEmpty
                ? fromUserMap['email']
                : (_ticketDetail['fromUserEmail'] ??
                      _ticketDetail['email'] ??
                      widget.ticket.raw['email']))
            ?.toString()
            .trim() ??
        '';
    if (rawEmail.isNotEmpty) {
      ...
      final email =
          (widget.ticket.raw['fromUser'] as Map)['email']?.toString().trim() ??
          '';
                            ...
                            InkWell(
                              ...
                              onTap: _updatingStatus
                                  ...
                                  : () async {
                                      ...
                                      if (chosen == selectedDropdownStatus)
                                        return;
