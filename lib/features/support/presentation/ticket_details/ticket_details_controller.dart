...
class TicketDetailsController {
  ...
  Future<void> loadTicketData() async {
    final detailsRequestId = ++state.detailsRequestId;
    final messagesRequestId = ++state.messagesRequestId;
    ...
    if (detailsRequestId != state.detailsRequestId) return;
    ...
    detailsResult.when(
      ...
      failure: (error) {
        state.errorMessage = ErrorPresenter.message(
          error,
          fallback: "Couldn't load ticket details.",
        );
      },
    );
    ...
    if (messagesRequestId != state.messagesRequestId) return;
    ...
    messagesResult.when(
      ...
      failure: (error) {
        ...
        state.errorMessage ??= ErrorPresenter.message(
          error,
          fallback: "Couldn't load ticket messages.",
        );
      },
    );
  ...
  Future<void> reloadMessages() async {
    final requestId = ++state.messagesRequestId;
  ...
  Future<void> sendMessage(BuildContext context) async {
    ...
    final requestId = ++state.sendRequestId;
    ...
    if (requestId != state.sendRequestId) return;
    ...
    result.when(
      ...
      failure: (error) {
        ...
        final message = ErrorPresenter.message(
          error,
          fallback: "Couldn't send message.",
        );
        if (context.mounted) {
  ...
  Future<void> updateStatus(BuildContext context) async {
    ...
    final requestId = ++state.statusRequestId;
    ...
    result.when(
      ...
      failure: (error) {
        ...
        final message = ErrorPresenter.message(
          error,
          fallback: "Couldn't update status.",
        );
        _showError(context, message);
      },
    );
