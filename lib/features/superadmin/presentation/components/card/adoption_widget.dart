...
class AdoptionGrowthBox extends ConsumerStatefulWidget {
  const AdoptionGrowthBox({super.key});

  @override
  ConsumerState<AdoptionGrowthBox> createState() => _AdoptionGrowthBoxState();
}

class _AdoptionGrowthBoxState extends ConsumerState<AdoptionGrowthBox> {
  String monthTab = "12M";
  ...
  Future<void> _loadGraph() async {
    ...
    if (!mounted) return;
    updateLocalUiState(this, () => _loadingGraph = true);

    try {

      final res = await ref.read(getSuperadminDashboardGatewayUseCaseProvider).getAdoptionGraph();
      if (!mounted) return;
      ...
      res.when(
        ...
        failure: (err) {
          ...
          if (!mounted) return;
          updateLocalUiState(this, () => _loadingGraph = false);
          if (_graphErrorShown) return;
          _graphErrorShown = true;

          final msg = "Couldn't load growth data.";
      ...
      child: Row(
        ...
        children: [
          SmallTab(
            ...
            onTap: () => updateLocalUiState(this, () => monthTab = "12M"),
          ),
          SizedBox(width: spacing),
          SmallTab(
            label: "6M",
            selected: monthTab == "6M",
            onTap: () => updateLocalUiState(this, () => monthTab = "6M"),
          ),
          SizedBox(width: spacing),
          SmallTab(
            label: "3M",
            selected: monthTab == "3M",
            onTap: () => updateLocalUiState(this, () => monthTab = "3M"),
          ),
          SizedBox(width: spacing),
          SmallTab(
            label: "All",
            selected: monthTab == "All",
            onTap: () => updateLocalUiState(this, () => monthTab = "All"),
          ),
        ],
      ),
    );
