...
class RecentActivityBox extends ConsumerStatefulWidget {
  const RecentActivityBox({super.key});

  @override
  ConsumerState<RecentActivityBox> createState() => _RecentActivityBoxState();
}

class _RecentActivityBoxState extends ConsumerState<RecentActivityBox> {
  ...
  Future<void> _loadRecentVehicles() async {
    ...
    if (!mounted) return;
    updateLocalUiState(this, () => _loadingRecentVehicles = true);

    try {

      final res = await ref.read(getSuperadminDashboardGatewayUseCaseProvider).getRecentVehicles();
      if (!mounted) return;
      ...
      res.when(
        ...
        failure: (err) {
          ...
          if (!mounted) return;
          updateLocalUiState(this, () => _loadingRecentVehicles = false);
  ...
  Future<void> _loadRecentTransactions() async {
    ...
    if (!mounted) return;
    updateLocalUiState(this, () => _loadingRecentTransactions = true);

    try {

      final res = await ref.read(getSuperadminDashboardGatewayUseCaseProvider).getRecentTransactions(limit: 10);
      if (!mounted) return;
      ...
      res.when(
        ...
        failure: (err) {
          if (kDebugMode) {            AppLogger.debug(
              '[Home] GET /superadmin/transactions status=error',
            );
          }
          if (!mounted) return;
          updateLocalUiState(this, () => _loadingRecentTransactions = false);
  ...
  Future<void> _loadRecentUsers() async {
    ...
    if (!mounted) return;
    updateLocalUiState(this, () => _loadingRecentUsers = true);

    try {

      final res = await ref.read(getSuperadminDashboardGatewayUseCaseProvider).getRecentUsers();
      if (!mounted) return;
      ...
      res.when(
        ...
        failure: (err) {
          if (kDebugMode) {            AppLogger.debug(
              '[Home] GET /superadmin/dashboard/recentusers status=error',
            );
          }
          if (!mounted) return;
          updateLocalUiState(this, () => _loadingRecentUsers = false);
