...
class _AllActivitiesScreenState extends ConsumerState<AllActivitiesScreen> {
  ...
  Future<void> _loadActivities() async {
    ...
    try {
      final dashboardGateway = ref.read(getSuperadminDashboardGatewayUseCaseProvider);

      switch (widget.activityType) {
        case "Vehicles":
          final res = await dashboardGateway.getVehicles(limit: 200);
          if (!mounted) return;
          res.when(
            success: (vehicles) {
              if (kDebugMode) {
        ...
        case "Transactions":
          _loadAdmins();
          final res = await dashboardGateway.getRecentTransactions(limit: 200);
          if (!mounted) return;
          res.when(
        ...
        case "Users":
          final res = await dashboardGateway.getRecentUsers();
          if (!mounted) return;
          res.when(
            success: (users) {
              if (kDebugMode) {
  ...
  void _applySearch(String query) {
    if (!mounted) return;
    updateLocalUiState(this, () => _searchQuery = query.trim());
  }
  ...
  Future<void> _loadAdmins() async {
    ...
    try {
      final res = await ref.read(getSuperadminDashboardGatewayUseCaseProvider).getAdmins(limit: 200);
      if (!mounted) return;
      res.when(
        success: (admins) {
          updateLocalUiState(this, () {
                          ...
                          itemBuilder: (_, i) {
                            if (i == 0) {
                              return ListTile(
                                ...
                                trailing: _selectedAdmin == null
                                    ...
                                    : null,
                                onTap: () {
                                  updateLocalUiState(this, () => _selectedAdmin = null);
                            ...
                            return ListTile(
                              ...
                              trailing: _selectedAdmin?.id == a.id
                                  ...
                                  : null,
                              onTap: () {
                                updateLocalUiState(this, () => _selectedAdmin = a);
                                Navigator.pop(context);
                              },
                            );
                          },
