...
class FleetOverviewBox extends ConsumerStatefulWidget {
  const FleetOverviewBox({super.key});

  @override
  ConsumerState<FleetOverviewBox> createState() => _FleetOverviewBoxState();
}

class _FleetOverviewBoxState extends ConsumerState<FleetOverviewBox> {
  ...
  Future<void> _loadCounts() async {
    ...
    if (!mounted) return;
    updateLocalUiState(this, () => _loadingCounts = true);

    try {

      final res = await ref.read(getSuperadminDashboardGatewayUseCaseProvider).getTotalCounts();
      if (!mounted) return;
      ...
      res.when(
        ...
        failure: (err) {
          if (kDebugMode) {            AppLogger.debug(
              '[Home] GET /superadmin/dashboard/totalcounts status=error message=${err.message}',
            );
          }
          if (!mounted) return;
          updateLocalUiState(this, () => _loadingCounts = false);
