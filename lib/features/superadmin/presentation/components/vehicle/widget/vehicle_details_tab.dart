...
class VehicleDetailsTab extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final ignitionSource = _safe(detail?.device.ignitionSource);
...
class DeleteVehicleBox extends ConsumerStatefulWidget {
  final String vehicleId;
  const DeleteVehicleBox({super.key, required this.vehicleId});

  @override
  ConsumerState<DeleteVehicleBox> createState() => _DeleteVehicleBoxState();
}

class _DeleteVehicleBoxState extends ConsumerState<DeleteVehicleBox> {
  ...
  Future<void> _onDeletePressed() async {
    ...
    try {
      ...
      final res = await ref.read(superadminVehicleGatewayUseCaseProvider).deleteVehicle(widget.vehicleId);
      if (!mounted) return;
      ...
      res.when(
        ...
        failure: (err) {
          if (!mounted) return;
          updateLocalUiState(this, () => _submitting = false);
          final msg = "Couldn't delete vehicle.";
          ScaffoldMessenger.of(
            context,
