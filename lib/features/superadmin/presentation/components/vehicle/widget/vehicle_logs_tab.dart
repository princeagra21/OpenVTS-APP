...
class VehicleLogsTab extends ConsumerStatefulWidget {
  final String? imei;
  ...
  @override
  ConsumerState<VehicleLogsTab> createState() => _VehicleLogsTabState();
}

class _VehicleLogsTabState extends ConsumerState<VehicleLogsTab> {
  ...
  Future<void> _pickDateRange() async {
    ...
    if (results != null && results.length == 2) {
      updateLocalUiState(this, () => _selectedRange = results);
      await _loadLogs();
    }
  }
  ...
  Future<void> _loadLogs() async {
    ...
    try {
      ...
      final res = await ref.read(superadminVehicleGatewayUseCaseProvider).getVehicleLogs(imei, from: _isoDate(_selectedRange[0]), to: _isoDate(_selectedRange[1]), limit: 100);
      if (!mounted) return;
      ...
      res.when(
        ...
        failure: (err) {
          ...
          _errorShown = true;
          final msg = "Couldn't load logs. Showing current view.";
