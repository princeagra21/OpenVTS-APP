...
class _AddVehicleScreenState extends ConsumerState<AddVehicleScreen> {
  ...
  AppCancellationHandle? _pricingPlansCancellationHandle;
  AppCancellationHandle? _pricingPlansCapabilityCancellationHandle;
  ...
  @override
  void dispose() {
    _pricingPlansCancellationHandle?.cancel('AddVehicleScreen disposed');
    _pricingPlansCapabilityCancellationHandle?.cancel('AddVehicleScreen disposed');
  ...
  Future<void> _initPricingPlans() async {
    _pricingPlansCapabilityCancellationHandle?.cancel(
      'Reload pricing plan capability',
    );
    final token = AppCancellationHandle();
    _pricingPlansCapabilityCancellationHandle = token;

    if (!mounted) return;
    updateLocalUiState(this, () => _loadingPricingPlans = true);
    ...
    try {
      ...
      capRes.when(
        success: (canAccess) {
          if (!mounted) return;
          if (!canAccess) {
            updateLocalUiState(this, () => _loadingPricingPlans = false);
  ...
  Future<void> _loadPricingPlans({bool startLoading = true}) async {
    _pricingPlansCancellationHandle?.cancel('Reload pricing plans');
    final token = AppCancellationHandle();
    _pricingPlansCancellationHandle = token;
                  ...
                  child: Column(
                    children: [
                      ...
                      StylishDropdown(
                        ...
                        onChanged:
                            (_pricingPlanNames.length == 1 &&
                                _pricingPlanNames.first == "No data")
                            ? null
                            : (v) => updateLocalUiState(this, () => _selectedPricingPlan = v),
                      ),
                      const SizedBox(height: 16),
                      ...
                      StylishDropdown(
                        ...
                        items: deviceTypes,
                        onChanged: (v) =>
                            updateLocalUiState(this, () => _selectedDeviceType = v),
                      ),
                      const SizedBox(height: 16),
                      ...
                      StylishDropdown(
                        ...
                        items: vehicleTypes,
                        onChanged: (v) =>
                            updateLocalUiState(this, () => _selectedVehicleType = v),
                      ),
                      const SizedBox(height: 32),
