import 'package:open_vts/features/superadmin/presentation/controllers/superadmin_vehicle_list_controller.dart';
...
class AdminVehiclesTab extends ConsumerStatefulWidget {
  final String adminId;

  const AdminVehiclesTab({super.key, required this.adminId});

  @override
  ConsumerState<AdminVehiclesTab> createState() => _AdminVehiclesTabState();
}

class _AdminVehiclesTabState extends ConsumerState<AdminVehiclesTab> {
  String _searchQuery = '';
  ...
  void initState() {
    ...
    _searchController.addListener(() {
      ...
      updateLocalUiState(this, () => _searchQuery = next);
    });
  }
  ...
  Future<void> _loadVehicles() async {
    await ref
        .read(superadminVehicleListControllerProvider.notifier)
        .loadVehicles(adminId: widget.adminId);
  }

  @override
  Widget build(BuildContext context) {
    final vehicleState = ref.watch(superadminVehicleListControllerProvider);
    final vehicles = vehicleState.items;
    final loading = vehicleState.isLoading;
    if (loading) {
    ...
    final filtered = _filteredVehicles(vehicles);
    return Container(
      width: double.infinity,
                    ...
                    Expanded(
                      child: PopupMenuButton<String>(
                        onSelected: (value) {
                          ...
                          updateLocalUiState(this, () => _selectedFilter = value);
                        },
  ...
  List<SuperadminVehicleListItem> _filteredVehicles(List<SuperadminVehicleListItem> vehicles) {
  ...
  Future<void> _exportCsv(List<SuperadminVehicleListItem> items) async {
    if (items.isEmpty) {
  ...
  void _showExportOptions(List<SuperadminVehicleListItem> items) {
  ...
  Future<void> _exportPdf(List<SuperadminVehicleListItem> items) async {
    if (items.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('No vehicles to export.')),
      );
  ...
  pw.Widget _buildVehiclesPdfTable({
    required List<SuperadminVehicleListItem> items,
  ...
  Widget _vehicleRow(BuildContext context, SuperadminVehicleListItem v) {
    final cs = Theme.of(context).colorScheme;
    final double screenWidth = MediaQuery.of(context).size.width;
