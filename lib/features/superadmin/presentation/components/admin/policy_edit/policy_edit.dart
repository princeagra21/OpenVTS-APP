...
class _PolicyEditScreenState extends ConsumerState<PolicyEditScreen> {
  ...
  late final _repo = ref.read(userPolicyRepositoryAdapterProvider);
  ...
  Future<void> _loadPolicies() async {
    ...
    try {
      final res = await _repo.getPolicies(cancelToken: token);
      if (!mounted) return;
      ...
      res.when(
        ...
        failure: (error) {
          ...
          final message =
              (LegacyErrorPresenter.isApiFailure(error) &&
  ...
  Future<void> _saveAll() async {
    ...
    try {
      final res = await _repo.updatePolicies(
        payload,
        cancelToken: token,
      );
      if (!mounted) return;
