...
class EditCompanyScreen extends ConsumerStatefulWidget {
  ...
  const EditCompanyScreen({super.key, required this.profile});
...
class _EditCompanyScreenState extends ConsumerState<EditCompanyScreen> {
  ...
  void _hydrateFromMap(Map<String, Object?> p) {
    Map<String, Object?>? companyFromList;
    final companies = p['companies'];
    if (companies is List && companies.isNotEmpty && companies.first is Map) {
      companyFromList = Map<String, Object?>.from(
    ...
    final merged = <String, dynamic>{
      ...
      if (p['company'] is Map)
        ...Map<String, Object?>.from((p['company'] as Map).cast()),
    };
  ...
  String _valueFromKeys(Map<String, Object?> map, List<String> keys) {
    ...
    if (company is Map) {
      final nested = Map<String, Object?>.from(company.cast());
      for (final key in keys) {
    ...
    if (companies is List && companies.isNotEmpty && companies.first is Map) {
      final first = Map<String, Object?>.from((companies.first as Map).cast());
      for (final key in keys) {
        final raw = first[key];
        if (raw == null) continue;
        final text = raw.toString().trim();
  ...
  Map<String, String> _socialMap(Map<String, Object?> map) {
    final out = <String, String>{};

    void absorb(Object? source) {
      if (source is! Map) return;
      final m = Map<String, Object?>.from(source.cast());
      m.forEach((k, v) {
  ...
  Future<void> _openSocialPicker() async {
    ...
    updateLocalUiState(this, () => _selectedSocial = picked);
  }
  ...
  Future<void> _loadCompanyDetails() async {
    ...
    if (adminId.isEmpty) return;
        try {
      final res = await ref.read(getSuperadminAdminDetailUseCaseProvider)(adminId);
      if (!mounted) return;
      res.when(
        success: (profile) {
          _hydrateFromMap(<String, Object?>{
            'id': profile.id,
  ...
  String _companyId() {
    final p = widget.profile.data;
    final companies = p['companies'];
    final Map<String, Object?>? firstCompany =
        companies is List && companies.isNotEmpty && companies.first is Map
        ? Map<String, Object?>.from((companies.first as Map).cast())
        : null;
    final candidates = [
      firstCompany?['id'],
  ...
  Future<void> _submit() async {
    if (ref.read(superadminAdminFormControllerProvider).isSubmitting) return;
    ...
    final payload = <String, Object?>{
      ...
      final updated = await ref.read(superadminAdminFormControllerProvider.notifier).updateCompany(
        payload,
      ...
      final message = ref.read(superadminAdminFormControllerProvider).errorMessage ?? "Couldn't update company.";
      _snackOnce(message);
  }
                ...
                child: SingleChildScrollView(
                  ...
                  child: Column(
                    children: [
                      ...
                      GestureDetector(
                        onTap: ref.watch(superadminAdminFormControllerProvider).isSubmitting ? null : _submit,
                        child: Container(
                          ...
                          child: Center(
                            child: ref.watch(superadminAdminFormControllerProvider).isSubmitting
                                ? const AppShimmer(
                                    width: 18,
                                    height: 18,
                                    radius: 9,
