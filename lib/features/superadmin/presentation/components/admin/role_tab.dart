...
import 'package:open_vts/features/admin/domain/entities/permission_matrix.dart';
import 'package:open_vts/features/admin/domain/entities/role_item.dart';
...
class _RolesTabState extends ConsumerState<RolesTab> {
  ...
  List<RoleItem> _normalizeRoles(List<Map<String, Object?>> rows) {
    final out = <RoleItem>[];
    final seen = <String>{};
  ...
  Future<void> _loadRoleAndPermissions() async {
    ...
    try {
      final gateway = ref.read(getSuperadminAdminGatewayUseCaseProvider);
      final rolesFuture = gateway.getRoles();
      final profileFuture = gateway.getAdminProfile(widget.adminId);
      ...
      if (rolesRes.isSuccess) {
        nextRoles.addAll(
          _normalizeRoles(rolesRes.valueOrNull ?? const <Map<String, Object?>>[]),
        );
      } else {
        if (!_errorShown) {
          _errorShown = true;
      ...
      if (profileRes.isSuccess) {
        final profileValue = profileRes.valueOrNull;
        if (profileValue is! AdminProfile) {
          throw StateError("Unexpected admin profile response.");
        }
        final AdminProfile p = profileValue;
