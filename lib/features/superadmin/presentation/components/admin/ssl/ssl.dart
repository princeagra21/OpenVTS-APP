...
class SSLManagementScreen extends ConsumerStatefulWidget {
  const SSLManagementScreen({super.key});

  @override
  ConsumerState<SSLManagementScreen> createState() => _SSLManagementScreenState();
}

class _SSLManagementScreenState extends ConsumerState<SSLManagementScreen> {
  ...
  Future<void> _loadSsl() async {
    ...
    try {
      final res = await _repo.getSslCertificates(cancelToken: token);
      if (!mounted) return;
