...
class _BrandingSettingsScreenState extends ConsumerState<BrandingSettingsScreen> {
  ...
  AppCancellationHandle? _uploadToken;
  late final _repo = ref.read(whiteLabelRepositoryAdapterProvider);
  ...
  Future<void> _loadBranding() async {
    ...
    if (!mounted) return;
    updateLocalUiState(this, () => _loadingBranding = true);

    try {
      final res = await _repo.getWhiteLabelBranding(
        cancelToken: token,
      );
      if (!mounted) return;
      ...
      res.when(
        ...
        failure: (err) {
          updateLocalUiState(this, () => _loadingBranding = false);
  ...
  Future<void> _saveBranding() async {
    ...
    try {
      final res = await _repo.updateWhiteLabelBranding(
  ...
  Future<void> _pickAndUploadAsset(String type) async {
    ...
    try {
      final res = await _repo.uploadBrandAsset(
        type: type,
        bytes: bytes,
        filename: file.name,
        cancelToken: token,
