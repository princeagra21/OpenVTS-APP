...
class _HomeScreenState extends ConsumerState<HomeScreen> {
  ...
  void _refreshBaseUrl() {
    _baseUrl = ref.read(superadminGatewayAppConfigProvider).baseUrl.trim();
  }

  Future<void> _loadAccessToken() async {
    final token = await ref.read(superadminGatewaySessionServiceProvider).readAccessToken();
    if (!mounted) return;
  ...
  Future<void> _loadProfile() async {
    ...
    try {
      ...
      res.when(
        success: (profile) => updateLocalUiState(this, () {
          _profile = profile;
          _loadingProfile = false;
        }),
        failure: (_) => updateLocalUiState(this, () {
          _profile = null;
          _loadingProfile = false;
        }),
      );
  ...
  Future<void> _confirmLogout() async {
    ...
    final sessionService = ref.read(superadminGatewaySessionServiceProvider);
  ...
  Map<String, dynamic> _toDynamicMap(Object? value) {
    ...
    if (value is Map) return <String, dynamic>{for (final entry in value.entries) entry.key.toString(): entry.value};
    return const <String, dynamic>{};
  }

  String _extractProfileImageUrl(SuperadminProfile? profile) {
    if (profile == null) return '';
    final raw = _toDynamicMap(profile.raw);
