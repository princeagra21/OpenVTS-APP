...
import 'package:open_vts/features/superadmin/presentation/controllers/superadmin_admin_list_controller.dart';
...
class _AdminScreenState extends ConsumerState<AdminScreen> {
  ...
  Future<void> _openAdminDetails(SuperadminAdminListItem admin) async {
    final id = admin.id;
    if (id.isEmpty) return;
    final changed = await context.push<bool>(
      AppRoutePaths.superadminAdminsDetails(id),
      extra: admin.isActive,
    );
    if (changed == true) {
  ...
  SuperadminAdminListItem _mapAdmin(SuperadminAdminListItem a) => a;

  Future<void> _loadAdmins({bool showShimmer = true}) async {
    await ref
        .read(superadminAdminListControllerProvider.notifier)
        .loadAdmins(limit: _pageSize);
  }
  ...
  Future<void> _openRecordsSheet() async {
    ...
    updateLocalUiState(this, () => _pageSize = selected);
    Future.microtask(_loadAdmins);
  }

  Future<void> _toggleAdminStatus({
    required SuperadminAdminListItem admin,
    required bool isActive,
  }) async {
    final updated = await ref
        .read(superadminAdminListControllerProvider.notifier)
        .updateStatus(admin, isActive);
    if (!mounted || updated) return;
    final message = ref.read(superadminAdminListControllerProvider).errorMessage ??
        "Couldn't update status.";
    ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text(message)));
  }

  Future<void> _loginAsAdmin(SuperadminAdminListItem admin) async {
    ...
    if (token != null && token.isNotEmpty) {
      await ref.read(superadminSessionServiceProvider).startImpersonation(token);
      if (!mounted) return;
      context.go(AppRoutePaths.adminHome);
      return;
    }
    final message = ref.read(superadminAdminListControllerProvider).errorMessage ?? 'Login failed.';
    ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text(message)));
  }

  Future<void> _confirmLoginAsAdmin(SuperadminAdminListItem admin) async {
    final name = _safeText(admin.name, fallback: 'this admin');
  ...
  Widget build(BuildContext context) {
    ...
    final listState = ref.watch(superadminAdminListControllerProvider);
    final admins = listState.items;
    final loadingAdmins = listState.isLoading;
    final adminsLoadFailed = !loadingAdmins && listState.errorMessage != null && admins.isEmpty;
    final statusSubmittingAdminIds = listState.updatingIds;
    final loginSubmittingAdminIds = listState.impersonatingIds;
    ...
    final showNoData = !loadingAdmins && filteredAdmins.isEmpty;
